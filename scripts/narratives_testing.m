...
vnames = {'src_subject_id', 'session_id','param_run_num','shuffled_run','param_counterbalance_ver',...
...
for trl = 1:size(countBalMat,1)
    ...
    T.event01_fixation_biopac(trl)        = biopac_linux_matlab(biopac, channel.fixation, 1);
    WaitSecs(jitter1);
    jitter1_end                           = biopac_linux_matlab(biopac, channel.fixation, 0);
    ...
    if countBalMat.isText(trl)
        ...
        text_time = showText(text_file,p);
        size(text_time);
        T.event02_text_biopac(trl)        = biopac_linux_matlab(biopac, channel.text, 1);
       % T.event02_administer_type{trl}    = 'text';

        %T.event02_administer_onset(trl) = text_time;
        biopac_linux_matlab(biopac, channel.text, 0);

    else
        audio_filename = [countBalMat.stimulus_filename{trl}];
        audio_file = fullfile(dir_audio, audio_filename);
        T.event02_audio_biopac(trl)        = biopac_linux_matlab(biopac, channel.audio, 1);
        ...
        try
            ...
            pahandle = PsychPortAudio('Open', device, [], 0, freq{trl}, nrchannels);
        catch
        ...
        biopac_linux_matlab(biopac, channel.audio, 0);
...
%        T.event02_administer_type{trl}    = {'audio'};
    ...
    T.event03_feel_biopac(trl)          = biopac_linux_matlab(biopac, channel.feel, 1);
    [trajectory, RT, buttonPressOnset] = circular_rating_output(4,p,rating_tex,'FEEL');
    biopac_linux_matlab(biopac, channel.feel, 0);
    ...
    T.event04_expect_biopac(trl)          = biopac_linux_matlab(biopac, channel.expect, 1);
    [trajectory, RT, buttonPressOnset] = circular_rating_output(4,p,rating_tex,'EXPECT');
    biopac_linux_matlab(biopac, channel.expect, 0);
...
T.param_end_biopac(:)                     = biopac_linux_matlab(biopac, channel.trigger, 0);
